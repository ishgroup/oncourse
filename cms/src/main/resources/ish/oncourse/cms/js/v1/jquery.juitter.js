...
(function($) {
	var conf = {},
		...
		var running=false;
		// Twitter API Urls
		apifMultipleUSER = "//search.twitter.com/search.json?from%3A";
		apifUSER = "//search.twitter.com/search.json?q=from%3A";
		apitMultipleUSER = "//search.twitter.com/search.json?to%3A";
		apitUSER = "//search.twitter.com/search.json?q=to%3A";
		apiSEARCH = "//search.twitter.com/search.json?q=";
	$.Juitter = {
		...
		conectaTwitter: function(e){
			...
			$.ajax({
				...
				dataType: 'jsonp',
				timeout: 1000,
				error: function(){$("#"+contDiv).html("Sorry, the news feed is currently down. We are working on it"); },
					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							if(i==0){
								...
								tultID = item.id;
								$("<ul></ul>").attr('id', 'twittList'+ultID).attr('class','twittList').prependTo("#"+contDiv);
							...
							if (item.text != "undefined") {
								...
								/* Hiding @Username output below -> this is the original code
								if(fromID=="image") mHTML="<a href='http://www.twitter.com/"+item.from_user+"'><img src='"+item.profile_image_url+"' alt='"+item.from_user+"' class='juitterAvatar' /></a> "+$.Juitter.textFormat(tweet)+"<a href='" + link + "' class='JRM' "+openLink+"><span class='time'>"+jQuery.timeago(item.created_at)+"</span></a>";
								else mHTML="<a href='http://www.twitter.com/"+item.from_user+"'>@"+item.from_user+":</a> "+$.Juitter.textFormat(tweet)+"<a href='" + link + "' "+openLink+"><span class='time'>"+jQuery.timeago(item.created_at)+"</span></a>";*/
								

								if(fromID=="image") mHTML= $.Juitter.textFormat(tweet)+"<a href='" + link + "' class='JRM' "+openLink+"><span class='time'>"+jQuery.timeago(item.created_at)+"</span></a>";
								else mHTML= $.Juitter.textFormat(tweet)+"<a href='" + link + "' "+openLink+"><span class='time'>"+jQuery.timeago(item.created_at)+"</span></a>";
