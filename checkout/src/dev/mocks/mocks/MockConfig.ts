...
export class MockConfig {
  ...
  createValidationError(formErrors = 3, field: string[]): ValidationError {
    return {
      formErrors: L.range(0, formErrors).map((i) => faker.hacker.phrase()),
      fieldsErrors: field.map((f, index) => ({ name: f, error: faker.hacker.phrase(), index })),
    };
  }
...
export const CreatePromiseReject = (data: any, code = 400): Promise<any> => Promise.reject({
  data,
  status: code,
} as AxiosResponse);
