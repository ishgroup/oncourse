...
export const AddContactToSummary: Epic<any, IshState> = (action$: ActionsObservable<any>, store: MiddlewareAPI<any, IshState>): Observable<any> => action$.ofType(ADD_CONTACT_TO_SUMMARY).flatMap((action: IAction<{ contact: Contact, uncheckItems?: boolean }>) => {
  const state: IshState = store.getState();

  const { type, parent, forChild } = state.checkout.contactAddProcess;
  const { contact } = action.payload;
  const payerId = parent?.id || contact.id;
  ...
  const uncheckContactItems: boolean = (action.payload.uncheckItems
