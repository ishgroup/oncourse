...
import { Contact, CourseClass, CourseClassPrice, Enrolment } from '../../../../model';
...
class EnrolmentComp extends React.Component<Props, any> {
  public render(): JSX.Element {
    const {
      enrolment, courseClass, contact, onChange, onChangeClass, onChangeFields, readonly, onAddNewClasses
    } = this.props;
    const divClass = classnames('row', 'enrolmentItem', { disabled: !enrolment.selected });
    const name = `enrolment-${contact.id}-${enrolment.classId}`;
    const title = `${courseClass.course.name}`;
    ...
    return (
      <div className={divClass}>
        ...
        <CustomFieldsForm
          ...
          onSubmit={() => undefined}
          initialValues={getFormInitialValues(enrolment.fieldHeadings)}
    ...
    <div className="col-xs-8 col-md-8 alignright">
      {(fee || Number(fee) === 0)
        ...
        <div className="text-right">
          <span className="fee-full fullPrice" style={feeStyle}>
            $
            {fee}
          </span>
        </div>
      ...
      {discountedFee !== null
        && (
        <div className="text-right">
          <span className="fee-discounted discountedPrice">
            $
            {discountedFee}
          </span>
        </div>
      ...
      {discount !== null
        && (
        <span style={{ display: 'none' }} className="discount">
          $
          {discount}
        </span>
        )}
...
const ClassDetailsLabel = (classItem: CourseClass) => {
  ...
  return (
    <div>
      ...
      && (
      <span>
        <span className="started">{start}</span>
&nbsp;-&nbsp;
        <span className="ended">{end}</span>
      </span>
      )}
      </em>
    </div>
