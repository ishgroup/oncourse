...
class CustomFields extends React.Component<any, any> {
  ...
  render() {
    const {
      headings, selected, onUpdate, form, handleSubmit, dispatch
    } = this.props;
...
const CustomFieldsForm = reduxForm({
  ...
  validate: (data, props: any) => {
    ...
    if (props.headings && props.headings.length) {
      props.headings.map((headings) => headings.fields.map((field, index) => ((field.mandatory && (field.dataType !== 'BOOLEAN' || field.key === 'isMale') && !data[toFormKey(field.key, index)])
