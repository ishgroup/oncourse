...
class CheckoutServiceV2 {
  ...
  public makePayment(paymentRequest: PaymentRequest, xValidateOnly: boolean, payerId: string, referer: string): Promise<PaymentResponse> {
    return this.checkoutApi.makePayment(paymentRequest,xValidateOnly,payerId,referer);
  }
}
