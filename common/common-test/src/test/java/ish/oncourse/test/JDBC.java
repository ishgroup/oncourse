 ...
 * JDBC 3 and JDBC 3 is a subset of JDBC 4. The base level for the Derby tests
 * is JSR 169.
 */
public class JDBC {
	...
	/**
	 * Rollback and close a connection for cleanup. Test code that is expecting
	 * Connection.close to succeed normally should just call conn.close().
	 * 
	 * <P>
	 * If conn is not-null and isClosed() returns false then both rollback and
	 * close will be called. If both methods throw exceptions then they will be
	 * chained together and thrown.
	 * 
	 * @throws SQLException
	...
	/**
	 * Drop a database schema by dropping all objects in it and then executing
	 * DROP SCHEMA. If the schema is APP it is cleaned but DROP SCHEMA is not
	 * executed.
	 * 
	 * TODO: Handle dependencies by looping in some intelligent way until
	 * everything can be dropped.
	 * 
	 * 
	 * 
	 * @param dmd
	...
	/**
	 * DROP a set of objects based upon a ResultSet from a DatabaseMetaData
	 * call.
	 * 
	...
	/**
	 * Convert byte array to String. Each byte is converted to a hexadecimal
	...
	/**
	 * Escape a non-qualified name so that it is suitable for use in a SQL query
	 * executed by JDBC.
	 */
	...
	private static String compressQuotes(String source, String quotes) {
		...
		/*
		 * Replace each occurrence with a single quote and begin the search for
		 * the next occurrence from where we left off.
		 */
		while (index != -1) {
	...
	public static String identifierToCNF(String id) {
		...
		if (id.charAt(0) == '"' && id.length() >= 3 && id.charAt(id.length() - 1) == '"') {
			...
			return compressQuotes(id.substring(1, id.length() - 1), "\"\"");
	...
	/**
	 * Escape a schama-qualified name so that it is suitable for use in a SQL
	 * query executed by JDBC.
	 */
