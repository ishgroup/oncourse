...
/**
 * Test Course->EntityRelation->Course
...
public class CayenneRelationTest {
	...
	public void test() throws NamingException, SQLException {
		...
		EntityRelation relation = context.newObject(EntityRelation.class);
		relation.setFromEntityWillowId(fromCourse.getId());
		relation.setFromEntityIdentifier("Course");
		relation.setToEntityWillowId(toCourse.getId());
		relation.setToEntityIdentifier("Course");
		relation.setCollege(college);

		context.commitChanges();

		String sql = String.format("INSERT INTO EntityRelation (collegeId, fromEntityIdentifier, fromRecordId, toEntityIdentifier, toRecordId) VALUES ( %d, 'Course', %d, 'Course', %d);\n" +
		...
		assertEquals(toCourse.getId(), fromCourse.getToCourses().get(0).getToEntityWillowId());

		execute(sql);

		fromCourse = SelectById.query(Course.class, fromCourse.getId()).selectOne(context);
		assertEquals(1, fromCourse.getToCourses().size());
		assertEquals(toCourse1.getId(), fromCourse.getToCourses().get(0).getToEntityWillowId());
	}
