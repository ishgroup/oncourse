...
class DateFunctionsTest {
    ...
    void test_toDateTime() {
        Date date = DateFunctions.toDate("2017-10-23 18:00:00")
        ZonedDateTime dateTime = DateFunctions.toDateTime(date, "Australia/Perth")
        Assert.assertEquals(2017, dateTime.getYear())
        Assert.assertEquals(Month.OCTOBER, dateTime.getMonth())
        Assert.assertEquals(23, dateTime.getDayOfMonth())
    ...
    void test_toTimeZone() {
        ...
        calendar.time = DateFunctions.toTimeZone(date, "Australia/Perth")
