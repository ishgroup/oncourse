...
public class SolrQueryBuilderTest {
    ...
    public void testCreate() throws UnsupportedEncodingException, ParseException {
        ...
        String value = URLDecoder.decode(solrQueryBuilder.create().toString(), "UTF-8");
        assertEquals("Commons parameters",  "qt=standard&fl=id,name,course_loc,score&start=0&rows=100&fq=+collegeId:1 +doctype:course end:[NOW TO *]&q={!boost b=$boostfunction v=$qq}&boostfunction=recip(max(ms(NOW-1YEAR/DAY, startDate), 0),1.15e-8,1,1)&qq=(*:*)&sort=score desc,startDate asc,name asc", value);
