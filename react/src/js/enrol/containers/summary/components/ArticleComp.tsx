...
class ArticleComp extends React.Component<Props, any> {
  public render(): JSX.Element {
    ...
    let warning = article.warnings && article.warnings.length ? this.props.article.warnings[0] : null;
    ...
    return (
        <div className={divClass}>
          <ItemWrapper title={product.name} name={name} error={error} warning={warning} selected={article.selected} item={article} contact={contact}
                       onChange={onChange}>
            <div/>
          </ItemWrapper>
          {article.selected &&
