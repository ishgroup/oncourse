...
class MembershipComp extends React.Component<Props, any> {
  public render(): JSX.Element {
    ...
    let warning = membership.warnings && membership.warnings.length ? this.props.membership.warnings[0] : null;
    ...
    <div className={divClass}>
      <ItemWrapper title={product.name} name={name} error={error} warning={warning} selected={membership.selected} item={membership} contact={contact}
                   onChange={onChange}> 
        <div/>
      </ItemWrapper>
      {membership.selected && 
