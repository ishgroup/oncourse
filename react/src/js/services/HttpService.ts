...
export class DefaultHttpService implements HttpService {
  ...
  GET<T>(path: string, config?: AxiosRequestConfig): Promise<T> {
    return this.client.get(path, config).then(payload => payload.data) as Promise<T>;
  }

  POST<T>(path: string, data?: any, config?: AxiosRequestConfig): Promise<T> {
    return this.client.post(path, data, config).then(payload => payload.data) as Promise<T>;
  }
  ...
  DELETE<T>(path: string, config?: AxiosRequestConfig): Promise<T> {
    return this.client.delete(path, config).then(payload => payload.data) as Promise<T>;
  }
}
