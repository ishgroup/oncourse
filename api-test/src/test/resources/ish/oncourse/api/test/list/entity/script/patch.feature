...
#       <--->
        ...
        * def scriptToUpdate = {"id":"#(id)","name":"script to testing PATCH method","description":"some description","enabled":false,"trigger":{"type":"Enrolment cancelled","entityName":null,"cron":null},"content":"//test"}
        ...
        "enabled":false,
        "trigger":{"type":"Enrolment cancelled","entityName":null,"cron":null},
        "content":"//test",
        "lastRun":[],
...
#       <--->  Scenario have been finished. Now change script to default:
    ...
    Scenario: (+) Update system script (advanced panels are present) by admin

#       <---> Find id for script with name "send enrolment":
        Given path ishPathPlain
        And param entity = 'Script'
        And param search = 'name == "send enrolment"'
        And param columns = 'name'
        When method GET
        Then status 200
        * def id = get[0] response.rows[0].id
...
#       <--->
        ...
        * def scriptToUpdate = {"id":"#(id)","name":"send enrolment UPD","description":"UPD","enabled":true,"trigger":{"type":"On create and edit","entityName":"Enrolment","cron":null},"content":"def run(args) {UPD}"}
        ...
        "enabled":true,
        "trigger":{"type":"On create and edit","entityName":"Enrolment","cron":null},
        "content":"#string",
        "lastRun":"#ignore",
        "keyCode":null,
        "createdOn":"#ignore",
...
#    Scenario: (+) Update script (Email panel) by admin
#    Scenario: (+) Update script (Email panel) by notadmin with access rights
#    Scenario: (-) Update script (Email panels) by notadmin without access rights
