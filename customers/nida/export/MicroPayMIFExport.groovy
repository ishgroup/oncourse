    ...
    if (pl.classCost?.repetitionType == ClassCostRepetitionType.PER_STUDENT_CONTACT_HOUR ||
     pl.classCost?.repetitionType == ClassCostRepetitionType.PER_TIMETABLED_HOUR) {
      ...
      csv << [
        ...
        "blank" : null,
        "value" : null,
        "account" : getCostAccount(pl),
        "effective date": pl.dateFor?.format("ddMMyyyy"),
        "job code": exportMap[pl.classCost?.courseClass?.incomeAccount?.accountCode],
        "end date": null
      ]
...
def getCostAccount(payLine) {
  ...
  if (state == "VIC") {
    return code.startsWith("O") ? "C-087-CASAL-${state}" : "C-088-CASAL-${state}"
  } else {
    return code.startsWith("O") ? "C-083-CASAL-${state}" : "C-084-CASAL-${state}"
  }
