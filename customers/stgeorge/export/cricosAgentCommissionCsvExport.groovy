records*.successAndQueuedEnrolments*.invoiceLine*.invoice*.paymentInLines.flatten().unique().findAll() { it ->
    it.payment.status == PaymentStatus.SUCCESS &&
            it.amount.isGreaterThan(ish.math.Money.ZERO) &&
            (it.payment.amount.isGreaterThan(ish.math.Money.ZERO) || it.payment.paymentMethod.type == PaymentType.CONTRA) &&
