records.each { Enrolment e ->
	csv << [
			...
			"studentMale"                       : e.student.contact.isMale,
			...
			"gender"                         	: e.student.contact.isMale ? "M" : (e.student.contact.isMale == null) ? "" : "F",
			"usi"                            	: e.student?.usi,
			"usiStatus"                      	: e.student?.usiStatus,
			"countryOfBirth"                 	: e.student?.countryOfBirth?.name,
			"languageSpokenAtHome"           	: e.student?.language?.name,
			"citizenship"                    	: e.student?.citizenship,
			"Invoice Number"                	: e.originalInvoiceLine.invoice.invoiceNumber,
			"Invoice Date"                  	: e.originalInvoiceLine.invoice.invoiceDate?.format("yyyy-MM-dd"),
			"Due Date"							: e.originalInvoiceLine.invoice.dateDue?.format("yyyy-MM-dd"),
			"Invoice line amount"           	: e.originalInvoiceLine.priceTotalIncTax?.toPlainString(),
			"Invoice line Tax"              	: e.originalInvoiceLine.totalTax?.toPlainString(),
			"Invoice line discount"         	: e.originalInvoiceLine.discountTotalExTax?.toPlainString(),
			"Invoice Owing"                 	: e.originalInvoiceLine.invoice.amountOwing?.toPlainString(),
			"How did you hear about us"			: e.student.contact.customFields.find { cf -> cf.customFieldType.key  == "howDidYouHearAboutThisCourse" }?.value,
	]
}
