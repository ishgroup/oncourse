...
function updateIncrementorState(node, state) {
    ...
    var arrayExpr = utils_1.unwrapParentheses(elementAccess.expression);
    if (state.arrayExpr.getText() !== arrayExpr.getText()) {
        // iterator used in array other than one iterated over
        state.onlyArrayReadAccess = false;
    }
    else if (utils_1.isAssignment(elementAccess.parent)) {
