...
export function mockTimetable() {
    this.getDates = (year, month) => {
        return generateArraysOfRecords(getDaysInMonth(new Date(year, month)), [{ name: "day", type: "number" }]).map(l => l.day);
    };
    ...
    this.getTimetableSessionsTags = (sessionIds) => {
        return [
            {
                Rozelle: "2d4d63",
