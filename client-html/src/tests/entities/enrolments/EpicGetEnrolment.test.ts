...
describe("Get enrolment epic tests", () => {
  it("EpicGetEnrolment should returns correct values", () => DefaultEpic({
    action: mockedApi => getEntityRecord(mockedApi.db.getEnrolment(1), "Enrolment"),
    epic: EpicGetEntityRecord,
    processData: (mockedApi, state) => getProcessDataActions(mockedApi.db.getEnrolment(1), "Enrolment", state)
  }));
});