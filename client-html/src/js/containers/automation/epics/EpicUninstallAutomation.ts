...
import { UNINSTALL_AUTOMATION } from "../actions";
...
const request: Request<any, { automation: CatalogItemType, entity: AutomationEntity }> = {
  ...
  processData: () => [showMessage({ message: "Automation uninstalled", success: true })],
  processError: response => FetchErrorHandler(response, "Failed to uninstall automation")
};

export const EpicUninstallAutomation: Epic<any, any> = Create(request);
