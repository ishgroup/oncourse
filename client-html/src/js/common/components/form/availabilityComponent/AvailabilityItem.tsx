...
const AvailabilityItem: React.FC<Props> = ({
  ...
  return (
    <Card id={`holidays-item-${index}`} className={threeColumn ? classes.threeColumnCard : "card"}>
      <Grid container>
        <Grid size={availabilityLayout[1].xs}>
          <Grid container rowSpacing={2} columnSpacing={3}>
            <Grid
              size={availabilityLayout[2].xs}
              className={clsx({
            ...
            <Grid size={availabilityLayout[3].xs}>
              <FormControlLabel
            ...
            <Grid size={availabilityLayout[4].xs}>
              <FormField
            ...
            <Grid size={availabilityLayout[5].xs}>
              <FormField
                type={allDay ? "date" : "dateTime"}
                timezone={timezone}
            ...
            <Grid size={availabilityLayout[6].xs} className={threeColumn ? "mt-2" : ""}>
              <FormField
                type="select"
                name={`${item}.repeat`}
            ...
            {field.repeat !== RepeatEnum.none && (
              <Grid size={availabilityLayout[7].xs}>
                <FormField
                  type="select"
                  name={`${item}.repeatEnd`}
                  label={$t('end_repeat')}
            ...
            {field.repeat !== RepeatEnum.none && field.repeatEnd === RepeatEndEnum.onDate && (
              <Grid size={availabilityLayout[8].xs}>
                <FormField
                  type="date"
                  timezone={timezone}
                  name={`${item}.repeatOn`}
            ...
            {field.repeat !== RepeatEnum.none && field.repeatEnd === RepeatEndEnum.after && (
              <Grid size={availabilityLayout[9].xs}>
                <FormField
        ...
        <Grid size={availabilityLayout[10].xs}>
          {!threeColumn && (
            <div className="d-flex errorColor">
              <div className="flex-fill"/>
              <Button size="small" color="inherit" onClick={onDelete.bind(null, field, index)}>
