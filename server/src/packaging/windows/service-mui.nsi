...
!define APPNAME "onCourseService"
...
Section
  ...
  CreateShortCut "$SMPROGRAMS\\onCourse\\Remove ${APPNAME}.lnk" "$INSTDIR\\uninstallService.exe"
  ...
  writeUninstaller $INSTDIR\\uninstallService.exe
  ...
  WriteRegDWORD HKLM "${ARP}" "EstimatedSize" "$0"
	...
	WriteRegStr HKLM "${ARP}" "UninstallString" "$\"$INSTDIR\\uninstallService.exe$\""
	WriteRegStr HKLM "${ARP}" "QuietUninstallString" "$\"$INSTDIR\\uninstallService.exe$\" /S"
...
section Uninstall
  ...
  Delete $INSTDIR\\uninstallService.exe
