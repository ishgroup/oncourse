...
class InvoiceUtilTest extends TestWithDatabase {
    ...
    void testSumPaymentLines() {
        ...
        Money sum1 = Money.of("" + Integer.parseInt("10101010", 2))
        Money sum2 = Money.of("" + Integer.parseInt("01010101", 2))
    ...
    void testSumInvoiceLinesNoTax() {
        ...
        invoiceLine1.setPriceEachExTax(Money.of("100"))
        ...
        invoiceLine2.setPriceEachExTax(Money.of("200"))
        invoiceLine2.setTaxEach(Money.ZERO)

        list.add(invoiceLine1)
        list.add(invoiceLine2)

        Money expectedValueExTax = Money.of("300")
    ...
    void testSumInvoiceLinesWithTax() {
        ...
        invoiceLine1.setQuantity(BigDecimal.ONE)
        invoiceLine1.setPriceEachExTax(Money.of("100"))
        invoiceLine1.setTaxEach(Money.of("10"))

        InvoiceLine invoiceLine2 = cayenneContext.newObject(InvoiceLine.class)
        invoiceLine2.setTax(tax)
        invoiceLine2.setQuantity(BigDecimal.ONE)
        invoiceLine2.setPriceEachExTax(Money.of("200"))
        invoiceLine2.setTaxEach(Money.of("20"))
        ...
        Money discount = Money.of("10")
        invoiceLine1.setDiscountEachExTax(discount)
        invoiceLine1.setTaxEach(invoiceLine1.getTaxEach().subtract(Money.of("1")))
        expectedValueExTax = expectedValueExTax.subtract(discount)
    ...
    void testUpdateAmountOwingWithoutCommits() {
        ...
        invoiceLine.setPriceEachExTax(Money.of("100"))
        invoiceLine.setTaxEach(Money.of("10"))
        invoiceLine.setInvoice(invoice)

        InvoiceUtil.updateAmountOwing(invoice)
        assertEquals(Money.of("110"), invoice.getAmountOwing(), "Checking amount owing")
        ...
        invoiceLine2.setPriceEachExTax(Money.of("50"))
        invoiceLine2.setInvoice(invoice)

        InvoiceUtil.updateAmountOwing(invoice)
        assertEquals(Money.of("360"), invoice.getAmountOwing(), "Checking amount owing")

        PaymentIn payment = cayenneContext.newObject(PaymentIn.class)
        payment.setAccountIn(aDebtors)
        payment.setAmount(Money.of("100"))
        ...
        pinLine.setAmount(Money.of("100"))

        InvoiceUtil.updateAmountOwing(invoice)
        assertEquals(Money.of("260"), invoice.getAmountOwing(), "Checking amount owing")

        payment.setAmount(Money.of("360"))
        pinLine.setAmount(Money.of("360"))
    ...
    void testUpdateAmountOwingWithCommits() {
        ...
        invoiceLine2.setTaxEach(Money.of("0"))
        invoiceLine2.setInvoice(invoice)
        ...
        payment2.setAmount(Money.of("260"))
        ...
        pinLine2.setAmount(Money.of("260"))
    ...
    void testUpdateAmountOwingWithoutCommitsPaymentInAndOut() {
        ...
        PaymentIn payment = cayenneContext.newObject(PaymentIn.class)
        payment.setAccountIn(account2)
        payment.setAmount(Money.of("460"))
        SetPaymentMethod.valueOf(PaymentMethodUtil.getCustomPaymentMethod(cayenneContext, PaymentMethod.class, PaymentType.CASH), payment).set()
        payment.setStatus(PaymentStatus.SUCCESS)
        payment.setPayer(contact)
        ...
        invoice.addToPaymentInLines(pinLine)
        // pinLine.setInvoice(invoice);
        pinLine.setAmount(Money.of("460"))
        pinLine.setAccountOut(account2)

        InvoiceUtil.updateAmountOwing(invoice)
        assertEquals(Money.of("100").negate(), invoice.getAmountOwing(), "Checking amount owing")

        PaymentOut payment2 = cayenneContext.newObject(PaymentOut.class)
        payment2.setAccountOut(account2)
        payment2.setAmount(Money.of("100"))
        SetPaymentMethod.valueOf(PaymentMethodUtil.getCustomPaymentMethod(cayenneContext, PaymentMethod.class, PaymentType.CASH), payment2).set()
        payment2.setStatus(PaymentStatus.SUCCESS)
        ...
        pinLine2.setAmount(Money.of("100"))
    ...
    void testAllocateSingleInvoice() {
        ...
        Money result = InvoiceUtil.invoiceAllocate(invoice, Money.of("110"), payment, list, false)
        assertEquals(Money.of("110"), result, "Checking money allocation for invoice")
        ...
        assertEquals(Money.of("110"), payment.getPaymentLines().get(0).getAmount(), "Checking money allocation for paymentline")

        // then just half
        result = InvoiceUtil.invoiceAllocate(invoice, Money.of("55"), payment, list, false)
        assertEquals(Money.of("55"), result, "Checking money allocation for invoice")
        assertEquals(1, invoice.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(1, payment.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(Money.of("55"), payment.getPaymentLines().get(0).getAmount(), "Checking money allocation for paymentline")

        // then actually types some wrong amount
        result = InvoiceUtil.invoiceAllocate(invoice, Money.of("150"), payment, list, false)
        assertEquals(Money.of("110"), result, "Checking money allocation for invoice")
        assertEquals(1, invoice.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(1, payment.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(Money.of("110"), payment.getPaymentLines().get(0).getAmount(), "Checking money allocation for paymentline")
        ...
        InvoiceUtil.updateAmountOwing(invoice)
        assertEquals(Money.of("55"), result, "Checking money allocation for invoice")
        assertEquals(Money.of("55"), invoice.getAmountOwing(), "Checking money owing for invoice")
        assertEquals(1, invoice.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(1, payment.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(Money.of("55"), payment.getPaymentLines().get(0).getAmount(), "Checking money allocation for paymentline")
        ...
        try {
            result = InvoiceUtil.invoiceAllocate(invoice, Money.of("10"), payment, list, false)
        ...
        // first just small
        result = InvoiceUtil.invoiceAllocate(invoice, Money.of("11"), payment2, list2, false)
        assertEquals(Money.of("11"), result, "Checking money allocation for invoice")
        ...
        assertEquals(Money.of("55"), payment.getPaymentLines().get(0).getAmount(), "Checking money allocation for paymentline")
        assertEquals(Money.of("11"), payment2.getPaymentLines().get(0).getAmount(), "Checking money allocation for paymentline")
        ...
        result = InvoiceUtil.invoiceAllocate(invoice, Money.of("55"), payment2, list2, false)
        assertEquals(Money.of("55"), result, "Checking money allocation for invoice")
        assertEquals(2, invoice.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(1, payment.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(1, payment2.getPaymentLines().size(), "Checking number of payment lines")
        assertEquals(Money.of("55"), payment.getPaymentLines().get(0).getAmount(), "Checking money allocation for paymentline")
        assertEquals(Money.of("55"), payment2.getPaymentLines().get(0).getAmount(), "Checking money allocation for paymentline")
    ...
    void testAllocateMultipleInvoices() {
        ...
        invoiceLine2.setQuantity(BigDecimal.ONE)
        invoiceLine2.setPriceEachExTax(Money.of("100"))
        invoiceLine2.setTaxEach(Money.of("10"))
        ...
        assertEquals(Money.of("220"), InvoiceUtil.amountOwingForPayer(contact), "Checking the amount owing")
        ...
        payment.setAccountIn(accountIn)
        SetPaymentMethod.valueOf(PaymentMethodUtil.getCustomPaymentMethod(cayenneContext, PaymentMethod.class, PaymentType.CASH), payment).set()
        payment.setAmount(Money.of("150"))
        ...
        if (pLine1.getAmount().compareTo(pLine2.getAmount()) > 0) {
            assertEquals(Money.of("110"), pLine1.getAmount(), "Checking money allocation for paymentline")
            assertEquals(Money.of("40"), pLine2.getAmount(), "Checking money allocation for paymentline")
        } else {
            assertEquals(Money.of("110"), pLine2.getAmount(), "Checking money allocation for paymentline")
            assertEquals(Money.of("40"), pLine1.getAmount(), "Checking money allocation for paymentline")
        }
        assertEquals(Money.of("150"), payment.getAmount(), "Checking payment total")
        assertEquals(Money.of("70"), InvoiceUtil.amountOwingForPayer(contact), "Checking the amount owing")
        ...
        invoiceLine3.setPriceEachExTax(Money.of("-63.64"))
        ...
        if (pLine1.getAmount().compareTo(pLine2.getAmount()) > 0) {
            assertEquals(Money.of("70"), pLine1.getAmount(), "Checking money allocation for paymentline")
            assertEquals(Money.of("-70"), pLine2.getAmount(), "Checking money allocation for paymentline")
        } else {
            assertEquals(Money.of("70"), pLine2.getAmount(), "Checking money allocation for paymentline")
            assertEquals(Money.of("-70"), pLine1.getAmount(), "Checking money allocation for paymentline")
        }
