...
class KeystoreGeneratorTest extends IshTestCase {
    ...
    void testKeystoreSaveAndLoad() throws Exception {
		try {
            assertFalse(new File(TEST_KEYSTORE_NAME).exists())
            ...
            assertNotNull(ks)
            assertNotNull(ks.getKey(TEST_ALIAS, TEST_KEY_PASS.toCharArray()))
            assertNotNull(ks.getCertificate(TEST_ALIAS))
            assertTrue(ks.getCertificate(TEST_ALIAS) instanceof X509Certificate)
            ...
            assertEquals(expectedExpiryDate, to)
            assertTrue(from.before(new Date()))
            assertTrue(to.after(new Date()))

            assertTrue(new File(TEST_KEYSTORE_NAME).exists())

            KeyStore ks2 = KeystoreGenerator.loadKeystore(TEST_KEYSTORE_NAME, TEST_KEYSTORE_PASS)
            assertNotNull(ks2)
            assertNotNull(ks2.getKey(TEST_ALIAS, TEST_KEY_PASS.toCharArray()))
            assertNotNull(ks.getCertificate(TEST_ALIAS))
            assertTrue(ks.getCertificate(TEST_ALIAS) instanceof X509Certificate)
    ...
    void testKeystoreAgainstKeytool() throws Exception {
        ...
        try {
            ...
            assertNotNull(ks2)
            assertNotNull(ks2.getKey(TEST_ALIAS, TEST_KEY_PASS.toCharArray()))
            assertNotNull(ks2.getCertificate(TEST_ALIAS))
            assertTrue(ks2.getCertificate(TEST_ALIAS) instanceof X509Certificate)
