...
class PaymentInLineTest extends CayenneIshTestCase {
    ...
    void testCreateMoneyVoucherTransactions() {
        ...
        assertTrue(context.select(SelectQuery.query(AccountTransaction.class)).isEmpty())
        ...
        assertEquals(new Money("-110.00"), assetTransactions.get(0).getAmount())
        ...
        assertEquals(new Money("110.00"), asset_expense.getAmount())
		...
		List<AccountTransaction> transactions2 = ObjectSelect.query(AccountTransaction.class)
				...
				.select(context)
        assertEquals(4, transactions2.size())

        List<AccountTransaction> assetTransactions2 = AccountTransaction.ACCOUNT.eq(debtorsAccount).filterObjects(transactions2)
        assertEquals(1, assetTransactions2.size())
        assertEquals(new Money("-110.00"), assetTransactions2.get(0).getAmount())
        ...
        assertEquals(1, liabilityTransactions2.size())
        assertEquals(new Money("-100.00"), liabilityTransactions2.get(0).getAmount())
        ...
        assertEquals(2, expenseTransactions2.size())
        ...
        assertEquals(new Money("110.00"), asset_expense2.getAmount())
        assertEquals(new Money("-100.00"), liability_expense2.getAmount())
    ...
    void testCourseVoucherTransactions() {
        ...
        List<AccountTransaction> assetTransactions = AccountTransaction.ACCOUNT.eq(debtorsAccount).filterObjects(transactions)
        assertEquals(1, assetTransactions.size())
        assertEquals(new Money("-220.00"), assetTransactions.get(0).getAmount())
        ...
        assertEquals(new Money("220.00"), asset_expense.getAmount())
        assertEquals(new Money("-100.00"), liability_expense.getAmount())
    }
}
