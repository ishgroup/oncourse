...
final class CourseClassUtil {
	 ...
	 */
	static List<Enrolment> getSuccessAndQueuedEnrolments(List<Enrolment> theEnrolments) {
	 ...
	 */
	static List<Enrolment> getRefundedAndCancelledEnrolments(List<Enrolment> theEnrolments) {
		if (theEnrolments == null || theEnrolments.size() == 0) {
			return theEnrolments
		}
	 ...
	 */
	static List<Enrolment> getCancelledEnrolments(List<Enrolment> theEnrolments) {
		if (theEnrolments == null || theEnrolments.size() == 0) {
			return theEnrolments
		}
	 ...
	 */
	static void addModuleToAllSessions(CourseClass courseClass, Module module) {
