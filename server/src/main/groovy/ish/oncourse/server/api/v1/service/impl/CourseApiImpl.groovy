...
class CourseApiImpl implements CourseApi {
    ...
    void duplicateCourse(List<Long> courseIds) {
        courseApiService.duplicateCourse(courseIds)
    }
    ...
    List<SaleDTO> getSellables(List<Long> courseIds) {
        ...
        return courseIds.collect {
            courseId -> relatedSellablesOf(context, courseId)
                    .collect {it.entityFromId = courseId}
    ...
    void update(Long id, CourseDTO courseDTO) {
        ...
        EntityRelationFunctions.updateRelatedEntities(cayenneService.newContext, id, Course.simpleName, courseDTO.relatedSellables)
    }
}
