...
/**
 * The superclass of all AVETMISS export lines.
 */
@CompileStatic
abstract class AvetmissLine {

    private static final Logger logger = LogManager.getLogger()
    private static final Format dateFormatter = new SimpleDateFormat("ddMMyyyy")
    public static final DateTimeFormatter localDateFormatter = DateTimeFormatter.ofPattern(DatePattern.ddMMyyyy)

    protected ExportJurisdiction jurisdiction
    private StringBuilder output = new StringBuilder(256)

    /**
     * A unique identifier for this row. Only one object with this identifier
     * can be put into the output. Any subsequent insertions will replace the
     * previous row.
     */
    protected String identifier

    AvetmissLine(ExportJurisdiction jurisdiction) {
        this.jurisdiction = jurisdiction
    }

    /**
     * Exports the file. This is overridden to do the actual work.
     */
    ...
    protected void append(int len, String data, String padCharacter, boolean alignRight) {
        if (data == null) {
            ...
            this.output.append(StringUtil.padField(len, data.toUpperCase(), padCharacter as char, alignRight))
        }
    }
    ...
    void appendCaseSensitive(int len, String data) {
        if (data == null) {
            ...
            this.output.append(StringUtil.padField(len, data, ' ' as char, false))
        }
    }
