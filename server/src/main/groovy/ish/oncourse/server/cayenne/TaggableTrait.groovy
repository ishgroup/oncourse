...
trait TaggableTrait implements Taggable{
    ...
    abstract boolean isNewRecord()

    abstract Class<? extends TagRelation> getTagRelationClass()

    abstract List<? extends TagRelation> getTaggingRelations()
    ...
    String getChecklistsColor() {
        def checklists = getChecklists();
        if (checklists.isEmpty())
            return null;
        def firstChecklist = checklists.get(0);
        def firstParent = firstChecklist.getParentTag();
        if (firstParent == null)
            firstParent = firstChecklist;
        String color = firstParent.getColour();
        def childChecklists = firstParent.getAllChildren().values();
        int allChildsNumber = childChecklists.size();
        long checkedChecklistsNumber = 0;
        childChecklists.each { checklist ->
    ...
    List<? extends Tag> getChecklists() {
        ...
        result.sort { o1, o2 -> o1.getCreatedOn().compareTo(o2.getCreatedOn()) }
        return result
    }
    ...
    boolean addTag(Tag tag) {
        if (tag != null && !getTags().contains(tag)) {
            if (tag.getRoot().getTagRequirement(this.getClass() as Class<? extends Taggable>) == null) {
                return false;
            }

            def taggedEntity = getContext().getEntityResolver().getObjEntity(getTagRelationClass());
            def joinEntity = getContext().getEntityResolver().getObjEntity(taggedEntity.getName());

            def relation = (TagRelation) getContext().newObject(joinEntity.getJavaClass());
            relation.setTag(tag);
            relation.setTaggedRelation(this as Taggable);
            return true;
        }
        return false;
    }
    ...
    boolean hasTag(String path) {
        return tagService.hasTag(this as Taggable, path, false);
    }
     ...
     */
    @API
    boolean hasTag(String path, boolean isSearchWithChildren) {
        return tagService.hasTag(this as Taggable, path, isSearchWithChildren);
    }
    ...
    private Tag getTagByFullPath(String path) {
        ...
        def tagNames = path.split("/");

        def rootExpression = Tag.NAME.eq(tagNames[0])
                .andExp(Tag.PARENT_TAG.isNull());
        def selectQuery = new SelectQuery<>(Tag.class, rootExpression);
    ...
    Set<Class<? extends Taggable>> getClassesForTags() {
        return Set.of(getClass() as Class<? extends Taggable>);
    }
}