...
allCertificates.each { certificate ->
    def eLsit = certificate.student.enrolments.findAll { enrolment ->
        enrolment.outcomes.contains(certificate.certificateOutcomes[0].outcome) &&
            ...
            enrolment.documents.find { d -> d.name == "${enrolment.courseClass.uniqueCode}_${certificate.student.contact.lastName}_${certificate.student.contact.firstName}_Certificate.pdf" } == null
    }

    if (eLsit.size() > 0) {

        def backGround = certificate.isQualification ? (QualificationType.SKILLSET_TYPE == certificate.qualification.type ? vetSkillsetBackground : vetQualificationBackground) : vetSoaBackground
