...
public class CourseClassEnrolmentTransformation extends PrintTransformation {

	private static Logger logger = LogManager.getLogger();

	public CourseClassEnrolmentTransformation() {
		setInputEntityName("CourseClass");
		setOutputEntityName("Enrolment");
	...
	public List<PersistentObjectI> applyTransformation(ObjectContext context, List<Long> sourceIds, Map<String, Object> additionalValues) {
		...
		for (int offset = 0; offset < sourceIds.size(); offset += getBatchSize()) {
			...
			ObjectSelect<PersistentObjectI> objectSelect = ObjectSelect.query(PersistentObjectI.class, "Enrolment")
			...
			if (Boolean.TRUE.equals(additionalValues.get(AdditionalParameters.BOOLEAN_FLAG.toString()))) {
				objectSelect.and(ExpressionFactory.inExp("status", EnrolmentStatus.STATUSES_LEGIT));
			}
