...
public class LazyApplicationComparisonNode extends LazyEntityComparisonNode {

    LazyApplicationComparisonNode(Op op) {
        super(op);
    }
    ...
    protected SimpleNode createNode() {
        ...
        var firstNameNode = createComparisionNode(pathString + Application.STUDENT.dot(Student.CONTACT.dot(Contact.FIRST_NAME)).getName(), value.getFirstName());
        var lastNameNode = createComparisionNode(pathString + Application.STUDENT.dot(Student.CONTACT.dot(Contact.LAST_NAME)).getName(), value.getLastName());
        var courseNameNode = createComparisionNode(pathString + Application.COURSE.dot(Course.NAME).getName(), value.getCourseName());
        var or = new ASTOr();
        var idx = 0;
