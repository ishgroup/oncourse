...
public class TimeMathOpConverter implements Converter<AqlParser.DateMathOpContext> {

    @Override
    public SimpleNode apply(AqlParser.DateMathOpContext op, CompilationContext ctx) {
        var value = evaluateAsDate(MathOp.from(op.mathOperator()), op, ctx);
        ctx.pushLevel();
    ...
    private Temporal evaluateAsDate(MathOp op, AqlParser.DateMathOpContext mop, CompilationContext ctx) {
    ...
    private TimeMathOpConverter.DateOpArguments evaluateArgs(CompilationContext ctx) {
        var arguments = ctx.popLevel();
        var result = new TimeMathOpConverter.DateOpArguments();
