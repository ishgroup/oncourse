...
public class LazySurveyComparisonNode extends LazyEntityComparisonNode {

    LazySurveyComparisonNode(Op op) {
        super(op);
    }
    ...
    protected SimpleNode createNode() {
        ...
        var firstNameNode = createComparisionNode(pathString +
                Survey.ENROLMENT
                .dot(Enrolment.STUDENT)
                .dot(Student.CONTACT)
                .dot(Contact.FIRST_NAME).getName(), value.getFirstName());
        var lastNameNode = createComparisionNode(pathString + Survey.ENROLMENT
                .dot(Enrolment.STUDENT)
                .dot(Student.CONTACT)
                .dot(Contact.LAST_NAME).getName(), value.getLastName());
        var courseNameNode = createComparisionNode(pathString + Survey.ENROLMENT
                .dot(Enrolment.COURSE_CLASS)
                .dot(CourseClass.COURSE)
                .dot(Course.NAME).getName(), value.getCourseName());
        var or = new ASTOr();
        var idx = 0;
