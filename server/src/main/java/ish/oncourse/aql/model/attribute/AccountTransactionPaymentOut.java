...
public class AccountTransactionPaymentOut implements SyntheticAttributeDescriptor {

    private EntityFactory factory;

    public AccountTransactionPaymentOut(EntityFactory factory) {
    ...
    public Optional<Entity> nextEntity() {
        return Optional.of(factory.createEntity(PaymentOut.class));
    }
        ...
        return new LazyExpressionNode() {
            ...
            public SimpleNode resolveParent(SimpleNode parent, List<SimpleNode> args, CompilationContext ctx) {
                ...
                if(args.get(0) != this || !(args.get(1) instanceof ASTPath)) {
                    ctx.reportError(-1, -1, "Invalid arguments in paymentOut property resolution.");
                }
                var basePath = ((ASTPath) args.get(1)).getPath();

                var idx = 0;
                var tempParent = parent;
                args.set(1, (SimpleNode) PaymentOutLine.PAYMENT_OUT.dot(getSearchProperty(basePath)).getExpression());
                ...
                var paymentOutLineIds = ObjectSelect.columnQuery(PaymentOutLine.class, PaymentOutLine.ID).where(tempParent).select(ctx.getContext());


                idx = 0;
                args.set(1, (SimpleNode) InvoiceLine.INVOICE.dot(Invoice.PAYMENT_OUT_LINES).dot(PaymentOutLine.PAYMENT_OUT).dot(getSearchProperty(basePath)).getExpression());
                ...
                return (SimpleNode) AccountTransaction.FOREIGN_RECORD_ID.in(paymentOutLineIds).andExp(AccountTransaction.TABLE_NAME.eq(AccountTransactionType.PAYMENT_OUT_LINE)).
