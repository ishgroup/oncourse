...
public class CreatePaylinesPerStudentContactHour extends AbstractPaylinesCreator {

	private static final Logger logger = LogManager.getLogger(CreatePaylinesPerStudentContactHour.class);

	private CreatePaylinesPerStudentContactHour(ClassCost classCost) {
		super(classCost);
	}

	public static CreatePaylinesPerStudentContactHour valueOf(ClassCost classCost) {
		return new CreatePaylinesPerStudentContactHour(classCost);
	}

	/**
	 * Create single payline for classCost with PER_STUDENT_CONTACT_HOUR repetition type.
	 * Budgeted quantity for such line is (valid enrolment count)*(payable classroom hours) - @see CourseClass getPayableClassroomHours(), @see CourseClass getValidEnrolmentCount()
	...
	public List<PayLine> createLines() {
		...
		logger.debug("paylines generate perHour, hours:");

		var count = classCost.getCourseClass().getPayableClassroomHours().multiply(BigDecimal.valueOf(classCost.getCourseClass().getValidEnrolmentCount()));
		...
		if (classCost.getPaylines().size() == 0
			...
			pl.setQuantity(count);
			pl.setBudgetedQuantity(count);
