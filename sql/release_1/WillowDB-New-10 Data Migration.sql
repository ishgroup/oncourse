...
INSERT INTO willow_college.College (id, primaryDomainId, collegeKey, isDeleted, isSslEnabled, isWebServicePaymentsEnabled, isWebSitePaymentsEnabled, isTestingWebServicePayments, isTestingWebSitePayments, requiresAvetmiss, created, modified, 
	...
	SELECT id, primaryDomainId, @siteKey, isDeleted, isSslEnabled, isWebServicePaymentsEnabled, isWebSitePaymentsEnabled, isTestingWebServicePayments, isTestingWebSitePayments, requiresAvetmiss, created, modified, 
		name, nationalProviderCode, billingCode, paymentGatewayAccount, paymentGatewayPass, paymentGatewayType, webServicesLogin, webServicesPass, webServicesSecurityCode, firstRemoteAuthentication, lastRemoteAuthentication 
	FROM oncourse_realdata_willow_college.College WHERE id = @collegeId AND isDeleted = 0;
INSERT INTO willow_college.CollegeDomain (id, collegeId, webSiteId, aliasedDomainId, isDeleted, hasSSL, created, modified, name, googleAnalyticsAccount, googleDirectionsFrom, subsiteCode) 
	SELECT id, collegeId, webSiteId, aliasedDomainId, isDeleted, hasSSL, created, modified, name, googleAnalyticsAccount, googleDirectionsFrom, subsiteCode
...
INSERT INTO willow_college.Student (angelId, collegeId, created, isDeleted, modified, id, countryOfBirthId, languageId, concessionType, disabilityType, englishProficiency, highestSchoolLevel, indigenousStatus, isOverseasClient, isStillAtSchool, priorEducationCode, yearSchoolCompleted, labourForceType)
	...
	WHERE tag.entityType = 'Student' AND s.id IN (SELECT studentId FROM Contact WHERE collegeId = @collegeId);
INSERT INTO willow_college.Tutor (angelId, collegeId, created, isDeleted, modified, id, startDate, finishDate, resume, resume_textile)
	...
	WHERE tag.entityType = 'Tutor' AND t.id IN (SELECT tutorId FROM Contact WHERE collegeId = @collegeId);
...
INSERT INTO willow_college.Invoice (amountOwing, collegeId, contactId, created, dateDue, id, invoiceDate, invoiceNumber, modified, source)
	...
	WHERE p.collegeId = @collegeId 
		...
		AND (p.studentId IN (SELECT id FROM Student));
...
INSERT INTO willow_college.StudentConcession (angelId, authorisationExpiresOn, authorisedOn, collegeId, concessionNumber, concessionTypeId, created, expiresOn, id, isDeleted, modified, studentId, timeZone)
	...
	WHERE collegeId = @collegeId AND isDeleted = 0
		AND concessionTypeId IN (SELECT id FROM ConcessionType WHERE collegeId = @collegeId)
		AND studentId IN (SELECT id FROM Student WHERE collegeId = @collegeId);
...
INSERT INTO willow_college.WebNodeContent (content, content_textile, created,modified, regionKey, webNodeId)
	SELECT content, content_textile, created, modified, 'content', id
	FROM oncourse_realdata_willow_college.WebNode WHERE isDeleted = 0 AND webSiteId IN (SELECT id FROM WebSite WHERE collegeId = @collegeId);
INSERT INTO willow_college.WebTheme (id, collegeId, angelId, isDeleted, name, themeKey, created, modified, description, description_textile)
	SELECT id, collegeId, angelId, isDeleted, name, themeKey, created, modified, description, description_textile
...
INSERT INTO willow_college.WebBlockTag (webBlockId, tagId)
	SELECT entityRecordId, tagId
	FROM oncourse_realdata_willow_college.WebTagged
	WHERE entityIdentifier = 'WebBlock' 
		AND entityRecordId IN (SELECT id FROM WebBlock WHERE webSiteId IN (SELECT id FROM WebSite WHERE collegeId = @collegeId));
INSERT INTO willow_college.WebNodeTag (webNodeId, tagId)
	SELECT entityRecordId, tagId
	FROM oncourse_realdata_willow_college.WebTagged
	WHERE entityIdentifier = 'WebNode' 
		AND entityRecordId IN (SELECT id FROM WebNode WHERE webSiteId IN (SELECT id FROM WebSite WHERE collegeId = @collegeId));
