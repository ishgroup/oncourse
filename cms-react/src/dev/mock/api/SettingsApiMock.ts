...
export function settingsApiMock() {
  ...
  this.api.onPost(API.SET_SKILLS_ON_COURSE_SETTINGS).reply(config => promiseResolve(
    config,
    this.db.settings.skillsOnCourse,
  ));
  ...
  this.api.onPost(API.SET_WEBSITE_SETTINGS).reply(config => promiseResolve(
    config,
    this.db.settings.website,
  ));
  ...
  this.api.onPost(API.SET_CHECKOUT_SETTINGS).reply(config => promiseResolve(
    config,
    this.db.settings.checkout,
  ));
}
