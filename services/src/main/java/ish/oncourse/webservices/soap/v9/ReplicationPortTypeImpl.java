...
import ish.oncourse.webservices.v9.stubs.replication.ErrorCode;
import ish.oncourse.webservices.v9.stubs.replication.FaultReason;
...
@WebService(endpointInterface = "ish.oncourse.webservices.soap.v9.ReplicationPortType", serviceName = "ReplicationService",
		portName = "ReplicationPort", targetNamespace = "http://repl.v9.soap.webservices.oncourse.ish/")
public class ReplicationPortTypeImpl implements ReplicationPortType {
	...
	public List<InstructionStub> getInstructions() {
		return PortHelper.convertV9InstructionsList(instructionService.getInstructions(SupportedVersions.V9));
	}
	...
	public ReplicationRecords getRecords() throws ReplicationFault {
		try {
			return PortHelper.getV9ReplicationRecords(replicationService.getRecords(SupportedVersions.V9));
	...
	public ReplicationResult sendRecords(ReplicationRecords replicationRecords) throws ReplicationFault {
		try {
			return PortHelper.getV9ReplicationResult(replicationService.sendRecords(replicationRecords));
		} catch (InternalReplicationFault e) {
			throw createReplicationFaultForException(e);
		}
	}
