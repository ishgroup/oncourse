...
import ish.oncourse.webservices.soap.v12.AuthFailure;
import ish.oncourse.webservices.soap.v12.ReplicationFault;
import ish.oncourse.webservices.soap.v12.ReplicationPortType;
import ish.oncourse.webservices.util.PortHelper;
import ish.oncourse.webservices.util.SupportedVersions;
import ish.oncourse.webservices.v12.stubs.replication.ErrorCode;
import ish.oncourse.webservices.v12.stubs.replication.FaultReason;
import ish.oncourse.webservices.v12.stubs.replication.InstructionStub;
import ish.oncourse.webservices.v12.stubs.replication.ReplicationRecords;
import ish.oncourse.webservices.v12.stubs.replication.ReplicationResult;
import ish.oncourse.webservices.v12.stubs.replication.TransactionGroup;
import ish.oncourse.webservices.v12.stubs.replication.UnreplicatedEntitiesStub;
...
@WebService(endpointInterface = "ish.oncourse.webservices.soap.v12.ReplicationPortType", serviceName = "ReplicationService",
		portName = "ReplicationPort", targetNamespace = "http://repl.v12.soap.webservices.oncourse.ish/")
public class ReplicationPortTypeImpl implements ReplicationPortType {
	...
	public List<InstructionStub> getInstructions() {
		return PortHelper.convertV12InstructionsList(instructionService.getInstructions(SupportedVersions.V12));
	}
	...
	public ReplicationRecords getRecords() throws ReplicationFault {
		try {
			return PortHelper.getV12ReplicationRecords(replicationService.getRecords(SupportedVersions.V12));
	...
	public ReplicationResult sendRecords(ReplicationRecords replicationRecords) throws ReplicationFault {
		try {
			return PortHelper.getV12ReplicationResult(replicationService.sendRecords(replicationRecords));
		} catch (InternalReplicationFault e) {
			throw createReplicationFaultForException(e);
		}
	}
	...
	public TransactionGroup getRecordByInstruction(String s) throws ReplicationFault {
		try {
			return PortHelper.getV12TransactionGroup(replicationService.getRecordByInstruction(s, SupportedVersions.V12));
		} catch (InternalReplicationFault e) {
			throw createReplicationFaultForException(e);
		}
	}
