...
@WebService(endpointInterface = "ish.oncourse.webservices.soap.v14.PaymentPortType", serviceName = "ReplicationService", portName = "PaymentPortType",
		targetNamespace = "http://repl.v14.soap.webservices.oncourse.ish/")
public class PaymentPortTypeImpl implements PaymentPortType {
	...
	public TransactionGroup getPaymentStatus(String sessionId) throws ReplicationFault {
		try {
			return PortHelper.getV14TransactionGroup(paymentPort.getPaymentStatus(sessionId, SupportedVersions.V14));
	...
	public TransactionGroup processRefund(TransactionGroup paymentOut) throws ReplicationFault {
		try {
			return PortHelper.getV14TransactionGroup(paymentPort.processRefund(paymentOut));
		} catch (InternalReplicationFault e) {
			throw ReplicationPortTypeImpl.createReplicationFaultForException(e);
		}
	}
	...
	public TransactionGroup getVouchers(TransactionGroup transactionGroup) throws ReplicationFault {
		try {
			return PortHelper.getV14TransactionGroup(paymentPort.getVouchers(transactionGroup, SupportedVersions.V14));
		} catch (InternalReplicationFault e) {
			throw ReplicationPortTypeImpl.createReplicationFaultForException(e);
		}
	}
	...
	public TransactionGroup processPayment(TransactionGroup transactionGroup, ParametersMap parametersMap) throws ReplicationFault {
		try {
			return PortHelper.getV14TransactionGroup(paymentPort.processPayment(transactionGroup, parametersMap));
		} catch (InternalReplicationFault e) {
			throw ReplicationPortTypeImpl.createReplicationFaultForException(e);
		}
	}
