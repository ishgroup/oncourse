...
public class JMXInitService implements IJMXInitService, Runnable {
	...
	public JMXInitService(ApplicationGlobals applicationGlobals,
		...
		try {
			newApplicationDataInstance = new ObjectName("ish.oncourse:type=ServicesApplicationData");
		...
		MBeanRegisterUtil.registerMbeanService(new ApplicationData("services", applicationGlobals, dataSource), applicationDataInstance);
