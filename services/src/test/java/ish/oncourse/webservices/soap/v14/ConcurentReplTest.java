...
public class ConcurentReplTest extends RealWSTransportTest {
	...
	protected void fillv14PaymentStubs(GenericTransactionGroup transaction, GenericParametersMap parametersMap) {
		...
		parametersMap.getGenericEntry().add(testEnv.getTestEnv().createEntry(String.format("%s_%d", ReplicationUtils.getEntityName(Enrolment.class), enrolmentStub.getAngelId()),
	...
	protected void fillv14ReplStubs(GenericTransactionGroup transaction) {
		List<GenericReplicationStub> stubs = transaction.getGenericAttendanceOrBinaryDataOrBinaryInfo();
		final Money hundredDollars = new Money("100.00");
		final Date current = new Date();
