...
public class QESuccessPaymentForPartiallyRefundedTest extends QEPaymentProcess7CaseGUITest {
	...
	public static void initTestServer() throws Exception {
		server = startRealWSServer(QE_SUCCESS_PAYMENT_FOR_PARTIALLY_REFUNDED_TEST_PORT);
	}
	...
	public void testSuccessQE() throws Exception {
		...
		fillV6PaymentStubsForCases7(transaction);
		//process payment
		...
		for (GenericReplicationStub stub : transaction.getGenericAttendanceOrBinaryDataOrBinaryInfo()) {
			if (stub instanceof GenericPaymentInStub) {
				...
				if (stub.getAngelId().equals(2l)) {
					assertNotEquals("Voucher status should be active",
				...
				if (stub.getAngelId().equals(3l)) {
					assertNotEquals("Article status should be active",
				...
				if (stub.getAngelId().equals(1l)) {
					assertNotEquals("Membership status should be active",
