...
public class QEPreviouslyKeepEnrolmentWithNewInvoiceTest extends RealWSTransportTest {
	...
	private void testRenderPaymentPageWithReverseInvoice(String sessionId) {
		...
		for (PaymentInLine paymentInLine : paymentInLines) {
			...
			assertTrue("InvoiceLines size should be 4", invoiceLines.size() == 4);
				...
				assertEquals("Enrolment status should be success", EnrolmentStatus.SUCCESS, enrolment.getStatus());
			}
		}
